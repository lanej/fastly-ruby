...
class Fastly::CreateResponseObject
  ...
  def mock
    ...
    if cache_condition_name
      cache_condition = find!(:conditions, service_id) do |vc|
        vc.values.find { |c| c[cache_condition_name] }
      end[cache_condition_name]
    ...
    if request_condition_name
      request_condition = find!(:conditions, service_id) do |vc|
        vc.values.find { |c| c[request_condition_name] }
      end[request_condition_name]
